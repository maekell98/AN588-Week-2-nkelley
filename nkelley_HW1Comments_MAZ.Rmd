---
title: "nkelley_FinalHomeworkCode_01"
author: "maekell98"
date: "9/17/2021"
output: html_document
---


**Challenge 1**

```{r Ulysses chunk}
v <- c("Stately", "plump", "Buck", "Mulligan", "came", "from", "the", "stairhead", "bearing", "a", "bowl", "of", "lather", "on", "which", "a", "mirror", "and", "a", "razor", "lay", "crossed", "NA", "NA")

m <- matrix(data = v, nrow =3, ncol = 8, byrow = FALSE)
m

x <- m[3,]
x

```

*instead of coercing it to a matrix, you can just use the seq() function: v[seq(1, length(v), 3)]*

*If you want to use the stringr function, you don't have to go through the trouble of putting quotations around each word in the sentence, you can use the str_split() function like so:* 
  library(stringr)
  v<-"Stately, plump Buck Mulligan came from the stairhead, bearing a bowl of lather
  on which a mirror and a razor lay crossed."
  v<-str_split(v, " ", simplify = TRUE) #separating each word 
  v[seq(from = 3, to = 22, by = 3)]
*Here, you are stll using the seq() function, but the str_split function first splits the string in the quotation by each word.-Mel*

**Challenge 2**

```{r matrix chunk}
m <- matrix(data = 1:80, nrow = 8, ncol = 10, byrow = FALSE)
m

x <- m[,c(2,3,6)] 
x
#part 1

x <- m[c(6,8),] 
x
#part 2

x <- m[c(2:6), c(2:9)]
x
#part3

```

*re part 3: Here you want to pull out all the numbers between the points in the matrix (2,2, all the way through 6,9), not just the block inbetween. You can do this by identifying each point in the matrix and then putting the colon between those points: x<-m[2,6]:m[2,9]. Remeber the commas indicate "row,column" and the colons mean "through" -Mel*



**Challenge 3** 

```{r array chunk}
a <- array(data = 400:1, dim = c(5, 5, 4, 4))
a

a[1, 1, 1, 2]
#returns 300

a[2, 3, 2, ]
#returns 364 264 164  64

a[1:5, 1:5, 3, 3]
#     [,1] [,2] [,3] [,4] [,5]
#[1,]  150  145  140  135  130
#[2,]  149  144  139  134  129
#[3,]  148  143  138  133  128
#[4,]  147  142  137  132  127
#[5,]  146  141  136  131  126
```



**Challenge 4** 

```{r phylogeny chunk}
Platyrrhini <- c("Cebidea", "Atelidae", "Pitheciidae")
Catarrhini <- c("Cercopithecidea", "Hylobatidae", "Hominidae")
Anthropoidea <- list(Platyrrhini, Catarrhini)
names(Anthropoidea) <- list("Platyrrhini", "Catarrhini")
Anthropoidea
Tarsioidea <- c("Tarsiidae")
Haplorhini <- list(Anthropoidea, Tarsioidea)
names(Haplorhini) <- list("Anthropoidea", "Tarsioidea")
Haplorhini
Lorisoidea <- c("Lorisidae", "Galagidae")
Lemuroidea <- c("Cheirogaleidae", "Lepilemuridae", "Indriidae", "Lemuridae", "Daubentoniidae")
Strepsirhini <- list(Lorisoidea, Lemuroidea)
names(Strepsirhini) <- list("Lorisoidea", "Lemuroidea")
Strepsirhini
Primates <- list(Haplorhini, Strepsirhini)
names(Primates) <- list("Haplorhini", "Strepsirhini")
Primates
```



**Challenge 5**

```{r matrix logical/factor chunk}
m3 <- matrix(data = c(3, 0, 1 ,23, 1, 2, 33, 1, 1, 42, 0, 1, 41, 0, 2), nrow = 5, ncol = 3, byrow = TRUE)

m3

m3.df <- as.data.frame(t(m3))

as.logical(m3.df$Col2)

as.factor(m3.df$Col3)

str(m3.df) 
```

*Something went wrong here when you made your matrix into a dataframe (the df has 5 columns when it should have the same dimensions as the matrix). I think it is the inclusion of the t in your as.data.frame() function. In the as.logical and as.factor functions, when you use the dollar sign, you need to use the exact name of the column in your dataframe (V2 and V3 in this case). Be sure to look at your dataframe after you make it so you can see this. You also want to assign it to its own column so that it is a permanent change. The correct code would be: m3.df$V2<-as.logical(m3.df$V2) #translates to: assign the column named "V2" in m3.df, as a logical function of column V2 in dataframe m3.df -Mel*





